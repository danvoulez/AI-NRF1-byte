...
fn make_medium() -> Value {
    ...
    m.insert(
        "agent_id".into(),
        Value::String("did:ubl:lab512#key-1".into()),
    );
    ...
    let evidence = vec![Value::Bytes(vec![
...
fn bench_decode(c: &mut Criterion) {
    ...
    group.bench_with_input(
        BenchmarkId::new("bincode", "medium"),
        &medium_bin,
        |b, v| b.iter(|| bincode::deserialize::<MediumDoc>(black_box(v)).unwrap()),
    );
